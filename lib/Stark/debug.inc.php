 ...
 */
function report_timers($return_output_string = false, $non_html_output = null) {
    global $BUG_ON, $BUG_TIME_I, $BUG_TIMERS, $BUG_TIMER_SUM, $BUG_TIMESEQ, $BUG_TIME_ST, $BUG_TIME_EN, $TIMER_LOG;
    if ( ! $BUG_ON && count($TIMER_LOG) == 0) return true;

	$return_string = '';
	
	if ( is_null( $non_html_output ) ) {
		$non_html_output = empty( $_SERVER['REQUEST_METHOD'] );
	}
	$html_output = ! $non_html_output; /// :)
...
#			}
        ...
        if ( $BUG_ON ) {
            ...
            ###  HTML table output
            if ( $html_output ) {
                $return_string .=  '<br/><div style="display: block; color: red; margin: 0; padding: 0; text-align: left"><table width="60%"><tr><td>';
                $return_string .=  "TIMERS:$nl";
                foreach ( $report_ary as $line ) {
                    $return_string .=  join("</td><td>\n", $line). $nl;
                }
                $return_string .=  "</td></tr></table></div>";
            }
            ###  Command-line output
            else {
                $f_widths = array( 35, 11, 10, 20, 17, false );
                foreach ( $report_ary as $line ) {
                    $join_ary = array(); foreach ($line as $i => $str) { $join_ary[] = bug_fill_width($str, $f_widths[$i]); }
                    $return_string .=  join(" ", $join_ary) ."\n";
                }
            }
        }
    }
